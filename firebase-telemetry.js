...
function normalizeGameName(gameElement) {
  const knownGames = ["ROBLOX", "CALL OF DUTY", "FREE FIRE", "MINECRAFT"];
  const text = (gameElement?.textContent || "").toUpperCase().replace(/\s+/g, " ").trim();
  return knownGames.find((name) => text.includes(name)) || text
    .replace(/TOCA PARA SELECCIONAR/g, "")
    .replace(/EN ESPERA/g, "")
    .replace(/ANALIZANDO COMPATIBILIDAD/g, "")
    .replace(/ANALIZANDO/g, "")
...
function bindExperienceEvents() {
  document.querySelectorAll(".game").forEach((game) => {
    game.addEventListener("click", async () => {
      ...
      selectedGame = normalizeGameName(game);
      await updateSession({
        game: selectedGame,
        selectedGame,
        gameSelectedAt: serverTimestamp()
      });
...
document.addEventListener("visibilitychange", () => {
  if (document.visibilityState === "hidden") {
    ...
    updateSession({ status: "inactive", currentScreen: lastScreen || "start-screen", game: selectedGame, selectedGame });
  } else if (sessionReady) {
    track("session_resumed", selectedGame, lastScreen || "start-screen");
    updateSession({ status: "active", currentScreen: lastScreen || "start-screen", game: selectedGame, selectedGame });
  }
});
